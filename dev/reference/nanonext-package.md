...
Guide to the implemented protocols for sockets:
...
Guide to the options that can be inspected and set using:
[opt](https://nanonext.r-lib.org/dev/reference/opt.md) / opt\<-

## Reference Manual

[`vignette("nanonext", package = "nanonext")`](https://nanonext.r-lib.org/dev/articles/nanonext.md)
...
NNG does not provide any other delivery or ordering guarantees: messages
may be dropped or reordered (some protocols, such as 'req' may offer
stronger guarantees by performing their own retry and validation
schemes).
...
dialers, this is the service address that is contacted, whereas for
