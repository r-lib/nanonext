...
2.  Passing a TLS configuration object to the ‘tls’ argument of
    [`listen()`](https://nanonext.r-lib.org/dev/reference/listen.md) or
...
**\[S\] Server process:**
[`reply()`](https://nanonext.r-lib.org/dev/reference/reply.md) waits for
a message, applies a function, and sends back the result. Started in a
...
performs async send/receive, returning immediately with a `recvAio`
object.
...
[`msleep()`](https://nanonext.r-lib.org/dev/reference/msleep.md) is an
...
[`ncurl_aio()`](https://nanonext.r-lib.org/dev/reference/ncurl_aio.md)
works anywhere that accepts a ‘promise’ from the promises package,
including Shiny ExtendedTask.
...
to request data multiple times. This enables polling frequencies that
...
[`recv()`](https://nanonext.r-lib.org/dev/reference/recv.md), as well as
their asynchronous counterparts
[`send_aio()`](https://nanonext.r-lib.org/dev/reference/send_aio.md) and
[`recv_aio()`](https://nanonext.r-lib.org/dev/reference/recv_aio.md) can
be used on Streams in the same way as Sockets. This affords a great deal
of flexibility in ingesting and processing streaming data.
...
and set options on Sockets, Contexts, Streams, Listeners, or Dialers.
See function documentation for available options.
...
Use [`stat()`](https://nanonext.r-lib.org/dev/reference/stat.md) to
access NNG’s statistics framework. Query Sockets, Listeners, or Dialers
for statistics such as connection attempts and current connections. See
