...
[`unresolved()`](https://nanonext.r-lib.org/reference/unresolved.md) may
also be used, which returns `TRUE` only if an Aio or Aio value has yet
to resolve and `FALSE` otherwise. This is suitable for use in control
