...
- [`stop_aio()`](https://nanonext.r-lib.org/reference/stop_aio.md) now
  resets the R interrupt flags
  ([\#194](https://github.com/r-lib/nanonext/issues/194)).
- The `cv` arguments at
  [`recv_aio()`](https://nanonext.r-lib.org/reference/recv_aio.md) and
  [`request()`](https://nanonext.r-lib.org/reference/request.md) have
  been simplified to accept just a ‘conditionVariable’ or NULL.
...
- A ‘recvAio’ returned by
  [`request()`](https://nanonext.r-lib.org/reference/request.md) now has
  an attribute `id`, which is the integer ID of the context passed to
  it.

##### Updates

- [`opt()`](https://nanonext.r-lib.org/reference/opt.md) for a boolean
  NNG option now correctly returns a logical value instead of an integer
  ([\#186](https://github.com/r-lib/nanonext/issues/186)).
...
- [`cv_reset()`](https://nanonext.r-lib.org/reference/cv.md) now
...
- The previous
  [`listen()`](https://nanonext.r-lib.org/reference/listen.md) and
  [`dial()`](https://nanonext.r-lib.org/reference/dial.md) argument
...
- [`serial_config()`](https://nanonext.r-lib.org/reference/serial_config.md)
  ...
  configurations.
- Adds [`ip_addr()`](https://nanonext.r-lib.org/reference/ip_addr.md)
  for returning the local network IPv4 address of all network adapters,
  named by interface.
- Adds [`pipe_id()`](https://nanonext.r-lib.org/reference/pipe_id.md)
  for returning the integer pipe ID for a resolved ‘recvAio’.
- Adds
...
- Adds
  ...
  [`recv()`](https://nanonext.r-lib.org/reference/recv.md) or
  [`recv_aio()`](https://nanonext.r-lib.org/reference/recv_aio.md).
- [`request()`](https://nanonext.r-lib.org/reference/request.md) gains
  integer argument `id`. This may be specified to have a special payload
  sent asynchronously upon timeout (to communicate with the connected
  party).
...
- [`nano()`](https://nanonext.r-lib.org/reference/nano.md) updated with
  the ‘poly’ protocol, with ‘pipe’ argument enabled for the send
  methods.
- [`write_cert()`](https://nanonext.r-lib.org/reference/write_cert.md)
...
- Adds [`monitor()`](https://nanonext.r-lib.org/reference/monitor.md)
  and
...
- Upgrades [`reply()`](https://nanonext.r-lib.org/reference/reply.md) to
...
- [`'opt<-'()`](https://nanonext.r-lib.org/reference/opt.md) now accepts
  the special option ‘serial’ for Sockets, which takes a configuration
  returned from
  [`serial_config()`](https://nanonext.r-lib.org/reference/serial_config.md).
...
- [`send_aio()`](https://nanonext.r-lib.org/reference/send_aio.md) now
  accept a Pipe to direct messages to a specific peer for supported
...
- Drops `base64enc()` and `base64dec()` in favour of those from the
  {secretbase} package.
- [`msleep()`](https://nanonext.r-lib.org/reference/msleep.md) now
...
- [`recv_aio()`](https://nanonext.r-lib.org/reference/recv_aio.md) and
  [`request()`](https://nanonext.r-lib.org/reference/request.md) add
  argument ‘cv’ allowing optional signalling of a condition variable.
  The separate functions `recv_aio_signal()` and `request_signal()` are
  deprecated.
...
- Performance enhancements for
  [`unresolved()`](https://nanonext.r-lib.org/reference/unresolved.md),
  [`call_aio()`](https://nanonext.r-lib.org/reference/call_aio.md) and
...
- Integrates with the `later` package to provide the foundation for
  ...
  - [`request()`](https://nanonext.r-lib.org/reference/request.md) and
    `request_signal()` modified internally to support conversion of
...
- [`stop_aio()`](https://nanonext.r-lib.org/reference/stop_aio.md) now
  causes the ‘aio’ to resolve to an ‘errorValue’ of 20 (Operation
  canceled) if successfully stopped.
- [`nng_error()`](https://nanonext.r-lib.org/reference/nng_error.md) now
  returns the error code combined with the message as a character
  string.
...
- An integer file descriptor is appended to ‘nanoSockets’ as the
  ...
  [`socket()`](https://nanonext.r-lib.org/reference/socket.md).
...
- Default behaviour of
  [`send()`](https://nanonext.r-lib.org/reference/send.md) and
  [`recv()`](https://nanonext.r-lib.org/reference/recv.md) aligned to
  non-blocking for both Sockets and Contexts (facilitated by synchronous
  context sends in NNG since v1.6.0).
...
- [`random()`](https://nanonext.r-lib.org/reference/random.md) now
...
- Introduces
  [`call_aio_()`](https://nanonext.r-lib.org/reference/call_aio.md), a
  user-interruptible version of
  [`call_aio()`](https://nanonext.r-lib.org/reference/call_aio.md)
  suitable for interactive use.
- Introduces [`wait_()`](https://nanonext.r-lib.org/reference/cv.md) and
  [`until_()`](https://nanonext.r-lib.org/reference/cv.md)
  user-interruptible versions of
  [`wait()`](https://nanonext.r-lib.org/reference/cv.md) and
  [`until()`](https://nanonext.r-lib.org/reference/cv.md) suitable for
  interactive use.
...
- [`until()`](https://nanonext.r-lib.org/reference/cv.md) updated to be
  identical to `.until()`, returning FALSE instead of TRUE if the
  timeout has been reached.
- [`reap()`](https://nanonext.r-lib.org/reference/reap.md) updated to no
  longer warn in cases it returns an ‘errorValue’.
- [`pipe_notify()`](https://nanonext.r-lib.org/reference/pipe_notify.md)
...
- Fixes regression in release 0.10.4 that caused a potential segfault
  using [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) with
  ‘follow’ set to TRUE when the server returns a missing or invalid
  relocation address.
...
- `.until()` contains revised behaviour for this synchronisation
  ...
  [`until()`](https://nanonext.r-lib.org/reference/cv.md) in a future
  package version.
...
- [`request()`](https://nanonext.r-lib.org/reference/request.md) adds
  logical argument ‘ack’, which sends an ack(nowledgement) back to the
  rep node upon a successful async message receive.
- [`reap()`](https://nanonext.r-lib.org/reference/reap.md) implemented
  as a faster alternative to
  [`close()`](https://nanonext.r-lib.org/reference/close.md) for
...
- [`random()`](https://nanonext.r-lib.org/reference/random.md) updated
  to use the Mbed TLS library to generate random bytes. Adds a ‘convert’
  argument for specifying whether to return a raw vector or character
  string.
...
- [`send()`](https://nanonext.r-lib.org/reference/send.md) and
  [`recv()`](https://nanonext.r-lib.org/reference/recv.md) over contexts
  now use more efficient synchronous methods where available.
...
- [`ncurl_aio()`](https://nanonext.r-lib.org/reference/ncurl_aio.md) has
  been separated into a dedicated function for async http requests.
...
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) argument
  ‘async’ is retired. Please use
  [`ncurl_aio()`](https://nanonext.r-lib.org/reference/ncurl_aio.md) for
  asynchronous requests.
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) now always
  returns the response message body at `$data` whether convert is TRUE
  or FALSE.
- The argument ‘keep.raw’ for all receive functions
  (previously-deprecated) is removed.
- [`cv_reset()`](https://nanonext.r-lib.org/reference/cv.md) and
  [`cv_signal()`](https://nanonext.r-lib.org/reference/cv.md) now both
  return invisible zero rather than NULL.
...
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) etc. gain
  higher performance raw to character conversion, resulting in the
  following changes:
...
- Enables secure TLS transports `tls+tcp://` and `wss://` for
  ...
  - [`listen()`](https://nanonext.r-lib.org/reference/listen.md) and
    [`dial()`](https://nanonext.r-lib.org/reference/dial.md) gain the
    argument ‘tls’ for supplying a TLS configuration object
  - [`write_cert()`](https://nanonext.r-lib.org/reference/write_cert.md)
...
- Fix cases where `base64enc()` failed for objects exceeding a certain
  size.
- [`stream()`](https://nanonext.r-lib.org/reference/stream.md) has been
  updated internally for additional robustness.
...
- ‘pem’ argument of
  [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md),
  [`ncurl_session()`](https://nanonext.r-lib.org/reference/ncurl_session.md)
  and [`stream()`](https://nanonext.r-lib.org/reference/stream.md)
...
- Implements [`cv_signal()`](https://nanonext.r-lib.org/reference/cv.md)
  and `timed_signal()` for signalling a condition variable, the latter
  after a specified time (from a newly-created thread).
- Implements
  [`.context()`](https://nanonext.r-lib.org/reference/dot-context.md), a
  performance alternative to
  [`context()`](https://nanonext.r-lib.org/reference/context.md) that
  does not create the full object.
- Adds utility `nanonext_version()` for providing the package version,
  NNG and mbed TLS library versions in a single string.
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) gains a
  ‘timeout’ argument.
...
- The name of the single argument to
  [`msleep()`](https://nanonext.r-lib.org/reference/msleep.md) has been
  changed to ‘time’ from ‘msec’.
- Functions
  [`pipe_notify()`](https://nanonext.r-lib.org/reference/pipe_notify.md),
...
- [`context()`](https://nanonext.r-lib.org/reference/context.md) gains
  the argument ‘verify’ with a default of TRUE. This adds additional
  protection to notably the
  [`request()`](https://nanonext.r-lib.org/reference/request.md) and
...
- [`until()`](https://nanonext.r-lib.org/reference/cv.md) now works as
  intended.
...
- Implements synchronisation primitives from the NNG library. Condition
  ...
  - adds core functions
    ...
    [`cv_value()`](https://nanonext.r-lib.org/reference/cv.md), and
...
- [`ncurl_session()`](https://nanonext.r-lib.org/reference/ncurl_session.md)
  ...
  upon error.
- [`listen()`](https://nanonext.r-lib.org/reference/listen.md) and
  [`dial()`](https://nanonext.r-lib.org/reference/dial.md) gain the new
  logical argument ‘error’ to govern the function behaviour upon error.
- Internal performance enhancements.
...
- Implements [`stat()`](https://nanonext.r-lib.org/reference/stat.md),
  an interface to the NNG statistics framework. Can be used to return
  the number of currently connected pipes for a socket, connection
  attempts for a listener/dialer etc.
- Implements
  [`parse_url()`](https://nanonext.r-lib.org/reference/parse_url.md),
...
- Using [`socket()`](https://nanonext.r-lib.org/reference/socket.md)
...
- [`opt()`](https://nanonext.r-lib.org/reference/opt.md) and
  [`'opt<-'()`](https://nanonext.r-lib.org/reference/opt.md) have been
  implemented as more ergonomic options getter and setter functions to
  replace `getopt()` and `setopt()`. These will error if the option does
  not exist / input value is invalid etc.
...
- For dialers, the ‘autostart’ argument to
  ...
  [`socket()`](https://nanonext.r-lib.org/reference/socket.md) and
  [`nano()`](https://nanonext.r-lib.org/reference/nano.md) now accepts
  NA for starting the dialer synchronously - this is less resilient if a
...
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) now returns
  redirect addresses as the response header ‘Location’. This is so that
  HTTP data can also be returned at `$data` where this is provided.
...
- The API has been re-engineered to ensure stability of return types:
  - [`socket()`](https://nanonext.r-lib.org/reference/socket.md),
    [`context()`](https://nanonext.r-lib.org/reference/context.md) and
    [`stream()`](https://nanonext.r-lib.org/reference/stream.md) will
    now error rather than return an ‘errorValue’. The error value is
    included in the error message.
  - [`send_aio()`](https://nanonext.r-lib.org/reference/send_aio.md) and
    [`recv_aio()`](https://nanonext.r-lib.org/reference/recv_aio.md) now
    always return an integer ‘errorValue’ at `$result` and `$data`
    respectively.
  - [`recv()`](https://nanonext.r-lib.org/reference/recv.md) and
    [`recv_aio()`](https://nanonext.r-lib.org/reference/recv_aio.md) now
    return an integer ‘errorValue’ at each of `$raw` and `$data` when
    ‘keep.raw’ is set to TRUE.
  - [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) now
...
- For functions that send and receive messages
  i.e. [`send()`](https://nanonext.r-lib.org/reference/send.md),
  [`send_aio()`](https://nanonext.r-lib.org/reference/send_aio.md),
...
- [`send()`](https://nanonext.r-lib.org/reference/send.md) no longer has
  a ‘…’ argument. This has had no effect since 0.6.0, but will now error
  if additional arguments are provided (please check and remove previous
  uses of the argument ‘echo’). Also no longer returns invisibly for
  consistency with
  [`recv()`](https://nanonext.r-lib.org/reference/recv.md).
- [`listen()`](https://nanonext.r-lib.org/reference/listen.md) and
  [`dial()`](https://nanonext.r-lib.org/reference/dial.md) now only take
  a socket as argument; for nano objects, the `$listen()` and `$dial()`
  methods must be used instead.
- [`nano()`](https://nanonext.r-lib.org/reference/nano.md) now creates a
  nano object with method `$context_open()` for applicable protocols.
  Opening a context will attach a context at `$context` and a
...
- Integer
  [`send()`](https://nanonext.r-lib.org/reference/send.md)/[`recv()`](https://nanonext.r-lib.org/reference/recv.md)
...
- `sha224()`, `sha256()`, `sha384()` and `sha512()` functions gain an
  ...
  character string.
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) gains the
  argument ‘follow’ (default FALSE) to control whether redirects are
...
- [`send()`](https://nanonext.r-lib.org/reference/send.md) now returns
  an integer exit code in all cases. The ‘echo’ argument has been
  replaced by ‘…’, and specifying ‘echo’ no longer has any effect.
- [`recv()`](https://nanonext.r-lib.org/reference/recv.md),
  [`recv_aio()`](https://nanonext.r-lib.org/reference/recv_aio.md) and
  [`request()`](https://nanonext.r-lib.org/reference/request.md) now
  default to ‘keep.raw’ = FALSE to return only the sent object.
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) argument
  ‘request’ renamed to ‘response’ for specifying response headers to
  return (to avoid confusion); new argument ‘follow’ (placed between
  ‘convert’ and ‘method’) controls whether redirects are followed, and
  there is no longer a user prompt in interactive environments.
...
- [`socket()`](https://nanonext.r-lib.org/reference/socket.md) and
  [`nano()`](https://nanonext.r-lib.org/reference/nano.md) now accept
  non-missing NULL ‘listen’ and ‘dial’ arguments, allowing easier
  programmatic use.
- Functions [`send()`](https://nanonext.r-lib.org/reference/send.md),
  [`recv()`](https://nanonext.r-lib.org/reference/recv.md),
  [`send_aio()`](https://nanonext.r-lib.org/reference/send_aio.md),
  [`recv_aio()`](https://nanonext.r-lib.org/reference/recv_aio.md),
  `setopt()`,
...
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) and
  [`stream()`](https://nanonext.r-lib.org/reference/stream.md) gain the
  argmument ‘pem’ for optionally specifying a certificate authority
  certificate chain PEM file for authenticating secure sites.
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) gains the
  argument ‘request’ for specifying response headers to return.
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) now returns
...
- `device()` gains a confirmation prompt when running interactively for
  more safety.
- Fixes issue with
  [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) that caused
  a 26 cryptography error with certain secure sites using SNI.
...
- [`msleep()`](https://nanonext.r-lib.org/reference/msleep.md) made safe
  (does not block) in case of non-numeric input.
- Internal performance optimisations.
...
- Adds [`mclock()`](https://nanonext.r-lib.org/reference/mclock.md),
  [`msleep()`](https://nanonext.r-lib.org/reference/msleep.md) and
  [`random()`](https://nanonext.r-lib.org/reference/random.md) utilities
  exposing the library functions for timing and cryptographic RNG
  respectively.
- [`socket()`](https://nanonext.r-lib.org/reference/socket.md) gains the
  ability to open ‘raw’ mode sockets. Please note: this is not for
...
- [`subscribe()`](https://nanonext.r-lib.org/reference/subscribe.md) and
  [`unsubscribe()`](https://nanonext.r-lib.org/reference/subscribe.md)
...
- Added convenience auxiliary functions
  [`is_nano()`](https://nanonext.r-lib.org/reference/is_aio.md) and
...
- New [`stream()`](https://nanonext.r-lib.org/reference/stream.md)
...
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) adds an
...
- Returned NNG error codes are now all classed as ‘errorValue’ across
  the package.
- Unified [`send()`](https://nanonext.r-lib.org/reference/send.md) and
  [`recv()`](https://nanonext.r-lib.org/reference/recv.md) functions,
  and their asynchronous counterparts
  [`send_aio()`](https://nanonext.r-lib.org/reference/send_aio.md) and
  [`recv_aio()`](https://nanonext.r-lib.org/reference/recv_aio.md), are
  now S3 generics and can be used across Sockets, Contexts and Streams.
- Revised ‘block’ argument for
  [`send()`](https://nanonext.r-lib.org/reference/send.md) and
  [`recv()`](https://nanonext.r-lib.org/reference/recv.md) now allows an
  integer value for setting a timeout.
- `send_ctx()` and `recv_ctx()` are deprecated and will be removed in a
  future package version - the methods for
  [`send()`](https://nanonext.r-lib.org/reference/send.md) and
  [`recv()`](https://nanonext.r-lib.org/reference/recv.md) should be
  used instead.
...
- [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md) adds a ‘…’
  argument. Support for HTTP methods other than GET.

##### Updates

- [`listen()`](https://nanonext.r-lib.org/reference/listen.md) and
  [`dial()`](https://nanonext.r-lib.org/reference/dial.md) now return
  (invisible) zero rather than NULL upon success for consistency with
  other functions.
...
- Implements full async I/O capabilities
  - [`send_aio()`](https://nanonext.r-lib.org/reference/send_aio.md) and
    [`recv_aio()`](https://nanonext.r-lib.org/reference/recv_aio.md) now
    return Aio objects, for which the results may be called using
    [`call_aio()`](https://nanonext.r-lib.org/reference/call_aio.md).
- New [`request()`](https://nanonext.r-lib.org/reference/request.md) and
  [`reply()`](https://nanonext.r-lib.org/reference/reply.md) functions
  implement the full logic of an RPC client/server, allowing processes
  to run concurrently on the client and server.
...
- New [`ncurl()`](https://nanonext.r-lib.org/reference/ncurl.md)
...
- All send and receive functions,
  e.g. [`send()`](https://nanonext.r-lib.org/reference/send.md)/[`recv()`](https://nanonext.r-lib.org/reference/recv.md),
