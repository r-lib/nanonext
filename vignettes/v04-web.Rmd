...
#> $headers
#> $headers$date
#> [1] "Mon, 21 Sep 2026 13:38:42 GMT"
#> 
#> 
#> $data
...
#> 
#> $headers$`Set-Cookie`
#> [1] "sails.sid=s%3AW_LwDFwcNB8ypfUnr9NgizA8zXA1HY8t.s5D3ZhNMzRub5w80rVWRH3sh1jragWDCQyYmlOqkKug; Path=/; HttpOnly, __cf_bm=1KQj.lQ49uXJQ1g8p.T8k4kmR3ay5S2t5OSlt2a.JdM-1789997922.8451235-1.0.1.1-9ybQd7uAnMm7ug4QdPD7PlITjMWcdh9WNPP3HI_OvgnnnmSkqN2SCEOKD2TXci7uhmwGwU2d_0fqsUnD14V0MvskuReERyiTehtfYpBvYEsBIB2Us.IlmjFvDJlMOGKq; HttpOnly; Secure; Path=/; Domain=postman-echo.com; Expires=Mon, 21 Sep 2026 14:08:42 GMT, _cfuvid=5uIuMBvpYqor6LPImBiDDRoP62..QmU.r23bPb.zRp4-1789997922.8451235-1.0.1.1-tyudCwPRKDYn.h_FcDgAwi_lXHguj6bet2snh.T.6eE; HttpOnly; SameSite=None; Secure; Path=/; Domain=postman-echo.com"
#> 
#> $headers$Server
#> [1] "cloudflare"
#> 
#> $headers$`CF-RAY`
#> [1] "a3e978c9ca40f400-LHR"
#> 
#> 
#> $data
#> [1] "{\"args\":{},\"headers\":{\"host\":\"postman-echo.com\",\"accept-encoding\":\"gzip, br\",\"x-forwarded-proto\":\"https\"},\"url\":\"https://postman-echo.com/get\"}"
...
#> $date
#> [1] "Mon, 21 Sep 2026 13:38:43 GMT"
...
#> $headers
#> $headers$Date
#> [1] "Mon, 21 Sep 2026 13:38:45 GMT"
#> 
#> $headers$`Content-Type`
#> [1] "application/json; charset=utf-8"
#> 
...
`$serve()` is a blocking call that starts the server, runs the event loop to process requests, and automatically closes the server on interrupt (Ctrl+C). For non-blocking use, call `$start()` and `$close()` separately, with `repeat run_event_loop()` to run the event loop manually.
...
nanonext implements the [`_server.yml` standard](https://posit-dev.github.io/plumber2/articles/server_yml.html) for R server frameworks. This allows a deployment platform to launch a nanonext server without knowing anything about its implementation: it reads the `_server.yml` file at the root of the deployment, installs the package named by `engine`, and calls its launch function.
...
- `constructor` is the path to an R file whose final expression evaluates to a handler or list of handlers, as created by `handler()`, `handler_ws()`, `handler_stream()` etc. The constructor must not create a server itself.
...
- `host` / `port`: the address to listen on. Values passed by the platform take precedence, followed by the `HOST` / `PORT` environment variables, then these values, falling back to `127.0.0.1:8080`.
- `tls`: arguments to `tls_config()`. If `server` is set, the server is served over HTTPS.

All paths are resolved relative to the `_server.yml` file. Reading the configuration requires the `yaml` package.

A complete, runnable example ships with the package:


``` r
list.files(system.file("examples", "server-yml", package = "nanonext"),
           recursive = TRUE)
```

Launching is a blocking call that serves until interrupted. This is normally done by the deployment platform, but can be tested locally:


``` r
nanonext:::launch_server("_server.yml")
```
