...
test_type("externalptr", s <- socket(listen = "tls+tcp://127.0.0.1:0", autostart = FALSE, tls = tls_config(server = cert$server)))
test_true(grepl(":0$", s$listener[[1]]$url))
test_zero(start(s$listener[[1]]))
test_false(grepl(":0$", s$listener[[1]]$url))
...
if (later && NOT_CRAN) {
  ...
  test_true(grepl(":0$", srv$url))
  test_zero(srv$start())
  test_equal(srv$state, "started")
  test_false(grepl(":0$", srv$url))
  base_url <- srv$url
  Sys.sleep(0.1)
