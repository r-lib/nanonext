...
test_error(n$opt("socket-name", NULL), "supported")
...
test_class("nano", bus)
test_true(suppressWarnings(listen(bus, url = "test", fail = "warn")) >= 3L)
test_error(listen(bus, url = "test", fail = "error"))
test_true(listen(bus, url = "test", fail = "none") >= 3L)
test_true(suppressWarnings(dial(bus, url = "test", fail = 1L)) >= 3L)
test_error(dial(bus, url = "test", fail = 2L))
test_true(dial(bus, url = "test", fail = 3L) >= 3L)
...
test_error(socket(dial = "test"))
test_error(socket(listen = "test"))
...
if (suppressWarnings(call_aio(ncaio)$status == 200L)) test_notnull(ncaio$data)
...
test_error(messenger("invalidURL"))
