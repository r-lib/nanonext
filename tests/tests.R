...
durld1 <- sprintf("inproc://%s", basename(tempfile()))
durl1 <- sprintf("inproc://%s", basename(tempfile()))
test_class("conditionVariable", dcv <- cv())
test_class("nanoSocket", dhost <- socket("req", listen = durld1))
opt(dhost, "req:resend-time") <- 0L
test_type("externalptr", disp <- .dispatcher_start(durl1, durld1, NULL, NULL, dseed, NULL, dcv))
test_identical(attr(disp, "url"), durl1)
...
test_class("nanoSocket", ddaemon <- socket("poly", dial = durl1))
...
durld2 <- sprintf("inproc://%s", basename(tempfile()))
durl2 <- sprintf("inproc://%s", basename(tempfile()))
test_class("conditionVariable", dcv <- cv())
test_class("nanoSocket", dhost <- socket("req", listen = durld2))
opt(dhost, "req:resend-time") <- 0L
test_type("externalptr", disp <- .dispatcher_start(durl2, durld2, NULL, NULL, dseed, 1, dcv))
dcap <- .dispatcher_capacity(disp)
test_equal(dcap[["capacity"]], 1)
test_equal(dcap[["used"]], 0)
test_equal(dcap[["peak"]], 0)
test_true(.dispatcher_try_gate(disp))
test_null(.dispatcher_stop(disp))
