...
test_class("recvAio", cs <- request(req$socket, data = TRUE, msgid = 12L))
test_notnull(cs$data)
test_true(recv(rep, block = 500))
test_zero(send(rep, TRUE, mode = 1L, block = 500))
test_type("externalptr", ctxn <- .context(rep))
...
test_true(!.unresolved(fakectx))
test_error(request(fakectx, data = "test"), "valid Socket or Context")
...
if (!interactive()) test_class("conditionVariable", cv <- cv())
if (!interactive()) f <- file("stdin", open = "r")
if (!interactive()) test_true(is_nano(reader <- read_stdin()))
if (!interactive()) test_zero(pipe_notify(reader, cv, remove = TRUE, flag = TRUE))
if (!interactive()) test_true(is_aio(r <- recv_aio(reader, mode = "raw", cv = cv)))
if (!interactive()) close(f)
if (!interactive()) test_type("logical", until(cv, 500L))
if (!interactive()) test_zero(close(reader))
if (!interactive()) test_equal(r$data, 7L)
test_true(.interrupt())
