...
test_zero(pipe_notify(poly, cv, add = TRUE, remove = TRUE))
...
test_true(wait(cv))
test_true(wait(cv))
test_zero(send(poly1, "one", block = 500))
test_zero(send(poly2, "one", block = 500))
raio1 <- recv_aio(poly, timeout = 500)
test_equal(raio1[], "one")
raio2 <- recv_aio(poly, timeout = 500)
test_equal(raio2[], "one")
p1 <- pipe_id(raio1)
p2 <- pipe_id(raio2)
test_false(p1 == p2)
test_zero(send_aio(poly, "one", timeout = 500, pipe = p1)[])
test_zero(send(poly, "two", block = 500, pipe = p2))
test_type("integer", send(poly, "two", block = FALSE, pipe = p2))
...
if (NOT_CRAN) {
  ...
  q_durld <- sprintf("inproc://%s", random(8))
  q_durl <- sprintf("inproc://%s", random(8))
  q_client <- socket("req", listen = q_durld)
  opt(q_client, "req:resend-time") <- 0L
  q_disp <- .dispatcher_start(q_durl, q_durld, NULL, NULL, stream, 100, NULL)
  q_daemon <- socket("poly", dial = q_durl)
  .dispatcher_wait(q_disp, 1L)
  test_type("raw", recv(q_daemon, mode = "raw", block = 2000))
  q_ctxA <- context(q_client)
  q_aioA <- request(q_ctxA, data = "A", id = q_disp)
  test_type("raw", recv(q_daemon, mode = "raw", block = 2000))
  while (.dispatcher_info(q_disp)[4L] < 1L) msleep(1)
  q_ctxB <- context(q_client)
  q_aioB <- request(q_ctxB, data = "B", id = q_disp)
  while (.dispatcher_info(q_disp)[3L] < 1L) msleep(1)
  q_ctxC <- context(q_client)
  q_aioC <- request(q_ctxC, data = "C", id = q_disp)
  while (.dispatcher_info(q_disp)[3L] < 2L) msleep(1)
  test_equal(.dispatcher_info(q_disp)[3L], 2L)
  test_true(.dispatcher_capacity(q_disp)[["used"]] > 0)
  test_true(stop_request(q_aioC))
  while (.dispatcher_info(q_disp)[3L] > 1L) msleep(1)
  test_equal(.dispatcher_info(q_disp)[3L], 1L)
  q_ctxD <- context(q_client)
  q_aioD <- request(q_ctxD, data = "D", id = q_disp)
  while (.dispatcher_info(q_disp)[3L] < 2L) msleep(1)
  test_zero(send(q_daemon, raw(13), mode = "raw", block = 2000))
  test_type("raw", recv(q_daemon, mode = "raw", block = 2000))
  while (.dispatcher_info(q_disp)[3L] > 1L) msleep(1)
  test_equal(.dispatcher_info(q_disp)[3L], 1L)
  test_false(stop_request(q_aioA))
  test_zero(send(q_daemon, raw(13), mode = "raw", block = 2000))
  test_type("raw", recv(q_daemon, mode = "raw", block = 2000))
  while (.dispatcher_info(q_disp)[3L] > 0L) msleep(1)
  test_zero(send(q_daemon, raw(13), mode = "raw", block = 2000))
  while (.dispatcher_info(q_disp)[4L] > 0L) msleep(1)
  test_zero(send(q_daemon, raw(13), mode = "raw", block = 2000))
  msleep(100)
  test_null(.dispatcher_stop(q_disp))
  test_zero(close(q_daemon))
  test_zero(close(q_client))

  b_durld <- sprintf("inproc://%s", random(8))
  b_durl <- sprintf("inproc://%s", random(8))
  b_client <- socket("req", listen = b_durld)
  opt(b_client, "req:resend-time") <- 0L
  b_disp <- .dispatcher_start(b_durl, b_durld, NULL, NULL, stream, NULL, NULL)
  b_daemon <- socket("poly", dial = b_durl)
  .dispatcher_wait(b_disp, 1L)
  test_type("raw", recv(b_daemon, mode = "raw", block = 2000))
  b_ctx <- context(b_client)
  b_aio <- request(b_ctx, data = "task", id = b_disp)
  test_type("raw", recv(b_daemon, mode = "raw", block = 2000))
  while (.dispatcher_info(b_disp)[4L] < 1L) msleep(1)
  test_true(stop_request(b_aio))
  test_equal(length(recv(b_daemon, mode = "raw", block = 2000)), 0L)
  test_null(.dispatcher_stop(b_disp))
  test_zero(close(b_daemon))
  test_zero(close(b_client))

  f_durld <- sprintf("inproc://%s", random(8))
  test_error(.dispatcher_start("foobar://invalid", f_durld, NULL, NULL, stream, NULL))
  test_error(.dispatcher_start(sprintf("inproc://%s", random(8)),
                               sprintf("inproc://%s", random(8)),
