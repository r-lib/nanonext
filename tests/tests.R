...
test_class("nanoSocket", pull1 <- socket("pull", listen = "inproc://pipeline"))
test_class("nanoSocket", push1 <- socket("push", dial = "inproc://pipeline", autostart = NA))
test_zero(send(push1, "pipeline message", block = 500))
test_equal(recv(pull1, block = 500), "pipeline message")
test_zero(close(push1))
test_zero(close(pull1))

test_class("nanoSocket", bus1 <- socket("bus", listen = "inproc://mesh"))
test_class("nanoSocket", bus2 <- socket("bus", dial = "inproc://mesh", autostart = NA))
test_class("nanoSocket", bus3 <- socket("bus", dial = "inproc://mesh", autostart = NA))
test_zero(send(bus1, "mesh data", block = 500))
test_equal(recv(bus2, block = 500), "mesh data")
test_equal(recv(bus3, block = 500), "mesh data")
test_zero(close(bus3))
test_zero(close(bus2))
test_zero(close(bus1))
...
if (NOT_CRAN) {
  ...
  stream_code <- sprintf('
    ...
    close(s)
    s <- stream(listen = "ws://127.0.0.1:25556/api", textframes = TRUE)
    close(s)
  ', certfile)
  script <- tempfile(fileext = ".R")
  ...
  test_zero(close(s))
  Sys.sleep(0.3)
  test_class("nanoStream", s <- stream(dial = "ws://127.0.0.1:25556/api", textframes = TRUE))
  test_zero(close(s))
  unlink(script)
  unlink(certfile)
}
