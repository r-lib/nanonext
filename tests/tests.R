...
test_class("ncurlAio", put1 <- ncurl_aio("https://postman-echo.com/put", method = "PUT", headers = list(`Content-Type` = "text/plain", Authorization = "Bearer token"), data = "test", response = TRUE, timeout = 3000L))
test_print(put1)
...
test_equal(nng_error(0L), "0 | Hunky dory")
...
test_null(msleep("a"))
test_null(msleep(-1L))
test_null(msleep(TRUE))
test_null(msleep(FALSE))
...
test_type("externalptr", tls_config(auth = FALSE))
test_type("externalptr", tls_config(auth = TRUE))
...
s_str <- socket("pair", listen = "inproc://string_mode_test")
s_str1 <- socket("pair", dial = "inproc://string_mode_test")
test_zero(send(s_str, "string mode", mode = "raw", block = 100))
test_class("recvAio", raio_str <- recv_aio(s_str1, mode = "string", timeout = 500))
test_equal(call_aio(raio_str)$data, "string mode")
test_zero(send(s_str, as.raw(c(0x41, 0x00, 0x42)), mode = "raw", block = 100))
test_class("recvAio", raio_str <- recv_aio(s_str1, mode = "string", timeout = 500))
test_type("raw", suppressWarnings(call_aio(raio_str)$data))
test_zero(close(s_str))
test_zero(close(s_str1))

s_sync <- socket("rep", listen = "inproc://sync_dial_test")
s_sync1 <- socket("req")
test_zero(dial(s_sync1, "inproc://sync_dial_test", autostart = NA))
test_zero(close(s_sync))
test_zero(close(s_sync1))

dseed <- c(10407L, 1L, 2L, 3L, 4L, 5L, 6L)
durld <- sprintf("inproc://%s", basename(tempfile()))
durl <- sprintf("inproc://%s", basename(tempfile()))
test_class("conditionVariable", dcv <- cv())
test_class("nanoSocket", dhost <- socket("req", listen = durld))
opt(dhost, "req:resend-time") <- 0L
test_type("externalptr", disp <- .dispatcher_start(durl, durld, NULL, NULL, dseed, NULL, dcv))
test_identical(attr(disp, "url"), durl)
test_identical(.dispatcher_info(disp), c(0L, 0L, 0L, 0L, 0L))
dcap <- .dispatcher_capacity(disp)
test_identical(names(dcap), c("used", "peak", "capacity"))
test_true(is.na(dcap[["capacity"]]))
test_equal(dcap[["used"]], 0)
test_equal(dcap[["peak"]], 0)
test_true(.dispatcher_try_gate(disp))
test_true(.dispatcher_gate(disp))
test_null(.dispatcher_wait(disp, 0L))

task_q <- raw(13); task_q[1L] <- as.raw(0x07); task_q[5L] <- as.raw(1L)
test_zero(send(dhost, task_q, mode = "raw", block = 2000))
Sys.sleep(0.1)
test_equal(.dispatcher_info(disp)[3L], 1L)
test_class("nanoSocket", ddaemon <- socket("poly", dial = durl))
.dispatcher_wait(disp, 1L)
test_equal(.dispatcher_info(disp)[1L], 1L)
test_equal(.dispatcher_info(disp)[2L], 1L)
test_type("raw", recv(ddaemon, mode = "raw", block = 2000))
test_type("raw", recv(ddaemon, mode = "raw", block = 2000))
Sys.sleep(0.1)
test_equal(.dispatcher_info(disp)[3L], 0L)
test_equal(.dispatcher_info(disp)[4L], 1L)
test_zero(send(ddaemon, raw(13), mode = "raw", block = 2000))
test_type("raw", recv(dhost, mode = "raw", block = 2000))
test_equal(.dispatcher_info(disp)[5L], 1L)

sync_task <- raw(13); sync_task[1L] <- as.raw(0x07); sync_task[4L] <- as.raw(0x01); sync_task[5L] <- as.raw(80L)
test_zero(send(dhost, sync_task, mode = "raw", block = 2000))
test_type("raw", recv(ddaemon, mode = "raw", block = 2000))
mark_resp <- raw(13); mark_resp[1L] <- as.raw(0x07); mark_resp[4L] <- as.raw(0x01)
test_zero(send(ddaemon, mark_resp, mode = "raw", block = 2000))
test_type("raw", recv(dhost, mode = "raw", block = 2000))
test_equal(length(recv(ddaemon, mode = "raw", block = 2000)), 0L)
Sys.sleep(0.1)
test_equal(.dispatcher_info(disp)[1L], 0L)
test_zero(close(ddaemon))

test_class("nanoSocket", didle <- socket("poly", dial = durl))
test_type("raw", recv(didle, mode = "raw", block = 2000))
.dispatcher_wait(disp, 1L)
test_equal(.dispatcher_info(disp)[1L], 1L)
test_zero(close(didle))
Sys.sleep(0.1)
test_equal(.dispatcher_info(disp)[1L], 0L)
...
.dispatcher_wait(disp, 1L)
test_type("raw", recv(ddaemon, mode = "raw", block = 2000))
task_busy <- raw(13); task_busy[1L] <- as.raw(0x07); task_busy[5L] <- as.raw(2L)
test_zero(send(dhost, task_busy, mode = "raw", block = 2000))
test_type("raw", recv(ddaemon, mode = "raw", block = 2000))
test_zero(close(ddaemon))
reset_reply <- recv(dhost, mode = "raw", block = 2000)
test_type("raw", reset_reply)
test_class("errorValue", reset_err <- unserialize(reset_reply))
test_equal(as.integer(reset_err), 19L)

test_null(.dispatcher_stop(disp))
test_type("integer", post_info <- .dispatcher_info(disp))
test_equal(length(post_info), 5L)
test_true(all(is.na(.dispatcher_capacity(disp))))
test_null(.dispatcher_gate(disp))
test_null(.dispatcher_try_gate(disp))
test_null(.dispatcher_wait(disp, 1L))
test_null(.dispatcher_stop(disp))
test_zero(close(dhost))
...
test_class("nanoSocket", dhost <- socket("req", listen = durld))
opt(dhost, "req:resend-time") <- 0L
test_type("externalptr", disp <- .dispatcher_start(durl, durld, NULL, NULL, dseed, 0.000001, dcv))
test_equal(.dispatcher_capacity(disp)[["used"]], 0)
test_true(.dispatcher_try_gate(disp))
task_q <- raw(13); task_q[1L] <- as.raw(0x07); task_q[5L] <- as.raw(1L)
test_zero(send(dhost, task_q, mode = "raw", block = 2000))
Sys.sleep(0.1)
test_true(.dispatcher_capacity(disp)[["used"]] > 0)
test_true(.dispatcher_capacity(disp)[["peak"]] > 0)
test_false(.dispatcher_try_gate(disp))
test_null(.dispatcher_stop(disp))
test_zero(close(dhost))

test_type("integer", inf <- .dispatcher_info(NULL))
test_equal(length(inf), 5L)
test_true(all(is.na(.dispatcher_capacity(NULL))))
test_null(.dispatcher_gate(NULL))
...
test_class("sendAio", dev <- device_aio(ds1, ds2))
test_type("logical", unresolved(dev))
test_true(unresolved(dev))
test_error(device_aio(1L, 1L), "`s1` is not a valid Socket")
test_error(device_aio(ds1, 1L), "`s2` is not a valid Socket")
dc1 <- socket("pair", dial = "inproc://device1")
dc2 <- socket("pair", dial = "inproc://device2")
test_zero(send(dc1, "relayed", mode = "raw", block = 500))
test_equal(recv(dc2, mode = "character", block = 1000), "relayed")
test_null(stop_aio(dev))
test_true(is_error_value(dev$result))
close(dc1)
close(dc2)
close(ds1)
close(ds2)
cs1 <- socket("pair", listen = "inproc://device3")
cs2 <- socket("pair", listen = "inproc://device4")
test_true(is_error_value(call_aio(device_aio(cs1, cs2))$result))
close(cs1)
close(cs2)
